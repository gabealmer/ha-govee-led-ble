...
PYTHON ?= python3
NODE := bash scripts/node-tool.sh node
NPM := bash scripts/node-tool.sh npm
...
FRONTEND_INPUTS := $(sort $(FRONTEND_SOURCE) $(FRONTEND_CONFIG) $(FRONTEND_CONTRACTS) scripts/generate-frontend.sh scripts/node-tool.sh .node-version mise.toml Makefile)
