...
class Provenance:
    ...
    def __post_init__(self) -> None:
        ...
        if self.source_revision is not None and self.source_revision < 1:
            raise EffectValidationError("source revision must be positive")
        if (self.parent_id is None) != (self.parent_revision is None):
            raise EffectValidationError("parent ID and revision must be provided together")
        if self.parent_revision is not None and self.parent_revision < 1:
            raise EffectValidationError("parent revision must be positive")
...
class LibraryItem:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> LibraryItem:
        ...
        provenance_raw = _required_mapping(raw, "provenance")
...
def _provenance_to_dict(provenance: Provenance) -> dict[str, JsonValue]:
    return {
        "source_kind": provenance.source_kind.value,
        "source_id": provenance.source_id,
        "source_revision": provenance.source_revision,
        "parent_id": str(provenance.parent_id) if provenance.parent_id is not None else None,
        "parent_revision": provenance.parent_revision,
    }


def _provenance_from_dict(raw: Mapping[str, Any]) -> Provenance:
    try:
        source_kind = SourceKind(_required_str(raw, "source_kind"))
    except ValueError as exc:
        raise EffectValidationError("unknown provenance source kind") from exc
    parent_id_raw = raw.get("parent_id")
    try:
        parent_id = None if parent_id_raw is None else UUID(str(parent_id_raw))
    except ValueError as exc:
        raise EffectValidationError("parent ID must be a UUID") from exc
    return Provenance(
        source_kind=source_kind,
        source_id=_optional_str(raw, "source_id"),
        source_revision=_optional_int(raw, "source_revision"),
        parent_id=parent_id,
        parent_revision=_optional_int(raw, "parent_revision"),
    )
