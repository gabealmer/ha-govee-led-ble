...
from .protocol import WHITE_BALANCE_POSITIONS, build_power
...
class EffectPreviewManager:
        ...
        *,
        ...
        if speed is None:
            if speed_index is not None:
                raise PreviewError("this scene does not expose a documented Speed control")
            resolved_speed = None
        else:
            resolved_speed = speed.default_index if speed_index is None else speed_index
            if not 0 <= resolved_speed < speed.option_count:
                raise PreviewError(f"scene speed index {resolved_speed} outside 0..{speed.option_count - 1}")
        request = _PreviewRequest(
            session_id=session_id,
            config_entry_id=config_entry_id,
            sequence=sequence,
            updated_at=updated_at,
            fingerprint=f"scene:{coordinator.model}:{scene_id}:{effect_id}:{resolved_speed}",
            generation=0,
            correlation_id=str(uuid4()),
            reassert=True,
    ...
    def _invalidate_observed_match(self, request: _PreviewRequest) -> None:
        ...
        if observed is None or observed.matched_operation_id is None:
            return
...
def _verification_expectations(
    ...
    if isinstance(compiled, CompiledEffect):
        if compiled.verification_strategy in {
            VerificationStrategy.UNPROVEN_H6199_SLOT,
            VerificationStrategy.WRITE_COMPLETED,
        }:
            return None
        if compiled.activation_mode is ActivationMode.SCENE:
