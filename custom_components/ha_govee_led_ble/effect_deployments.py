...
class DeploymentRecord:
    ...
    def from_dict(cls, raw: Mapping[str, Any]) -> DeploymentRecord:
        ...
        return cls(
            ...
            progress_current=_required_int(raw, "progress_current"),
            progress_total=_required_int(raw, "progress_total"),
        )
...
class EffectDeploymentRepository:
    ...
    async def async_load(self) -> DeploymentSnapshot:
        async with self._lock:
            ...
            interrupted = {
                str(record.operation_id): replace(
                    record,
                    phase=DeploymentPhase.INTERRUPTED,
