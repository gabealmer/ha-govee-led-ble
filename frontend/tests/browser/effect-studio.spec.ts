...
test("modal dialogs contain focus and lock background scrolling", async ({
  page,
}) => {
  const studio = await openStudio(page);
  const categories = studio.getByRole("complementary", {
  ...
  const saveButton = studio
    .locator(".editor")
    .getByRole("button", { name: "Save", exact: true });
  await saveButton.click();
  const saveDialog = studio.getByRole("dialog", { name: "Save effect" });
  const name = saveDialog.getByLabel("Effect name");
  const confirmSave = saveDialog.getByRole("button", { name: "Save effect" });
  await expect(studio.locator("main.studio")).toHaveAttribute("inert", "");
  await expect
    .poll(() =>
      page.evaluate(() => ({
  ...
  await saveDialog.getByRole("heading", { name: "Save effect" }).click();
  await expect(saveDialog).toBeFocused();
  await saveDialog.press("Shift+Tab");
  await expect(confirmSave).toBeFocused();
  await confirmSave.press("Tab");
  await expect(name).toBeFocused();
  ...
  const deleteButton = studio
    .locator(".editor-heading .actions")
    .getByRole("button", { name: "Delete", exact: true });
  await deleteButton.click();
  const deleteDialog = studio.getByRole("dialog", {
    name: "Delete effect?",
  });
...
test("Painted effects keep multiple brushes and their picker visible", async ({
  ...
  await expect(
    studio
      .locator(".background-colour govee-colour-picker")
      .getByRole("button", { name: /^Use #/ })
      .first(),
  ).toHaveAccessibleName("Use #ff9f0a");
  await expect(
...
test("palette scenes apply native identity and authored saved or unsaved definitions", async ({
  ...
  const sceneDelete = reloadedBrowser.getByRole("button", { name: "Delete" });
  await sceneDelete.click();
  let deleteDialog = studio.getByRole("dialog", { name: "Delete effect?" });
  await deleteDialog.getByRole("button", { name: "Cancel" }).click();
  await expect(sceneDelete).toBeFocused();

  await sceneDelete.click();
  deleteDialog = studio.getByRole("dialog", { name: "Delete effect?" });
...
test("advanced layer and palette keyboard focus follows edits", async ({
  ...
  await layerActions.getByRole("button", { name: "Copy layer" }).press("Tab");
  const deleteLayer = layerActions.getByRole("button", {
    name: "Delete layer",
  });
  await expect(deleteLayer).toBeFocused();
  await deleteLayer.press("Tab");
  await expect(layerActions).toHaveCount(0);
  ...
  await expect(
    dialog.getByRole("button", { name: /^Use #/ }).first(),
  ).toBeFocused();
  ...
  await dialog.getByLabel("Custom colour").focus();
  await dialog.getByLabel("Custom colour").press("Tab");
  const addColour = palette.getByRole("button", { name: "Add colour" });
  await expect(addColour).toBeFocused();
  await addColour.press("Tab");
  await expect(dialog).toHaveCount(0);
  swatches = palette.getByRole("button", { name: /Edit colour/ });
  await swatches.nth(2).click();
  await expect(
