...
export interface AdvancedRangeScale extends LinearRange {
  suffix?: string;
}

export const ADVANCED_RANGE_SCALES = {
  bytePercentage: {
    displayMinimum: 0,
    displayMaximum: 100,
    rawMinimum: 0,
...
} as const satisfies Record<string, AdvancedRangeScale>;

export function displayValueToRaw(
  value: number,
  range: LinearRange,
): number {
  const displayValue = clampInteger(
    value,
    range.displayMinimum,
    range.displayMaximum,
  );
  return (
    Math.floor(
      ((displayValue - range.displayMinimum) *
        (range.rawMaximum - range.rawMinimum)) /
        (range.displayMaximum - range.displayMinimum),
    ) + range.rawMinimum
  );
}

export function rawValueToDisplay(
  value: number,
  range: LinearRange,
): number {
  const rawValue = clampInteger(
    value,
    range.rawMinimum,
    range.rawMaximum,
  );
