...
export function effectContentAvailable(
  context: CustomEffectListContext,
  ...
  if (
    ...
    return content.model === context.model;
  }
  if (content.kind === "scene_layered") {
    return content.template.sku === context.model;
  }
  return customEffectKindAvailable(context, content.kind);
}
