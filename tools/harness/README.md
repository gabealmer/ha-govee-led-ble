...
The per-device config-entry ID is written as soon as Home Assistant creates the entry, before the harness waits for
`loaded`.  A later `up` validates and resumes that exact enabled integration entry after `setup_retry`.  If the container
config has no recorded ID and the flow reports `already_configured`, recovery succeeds only when the isolated Home
Assistant config contains exactly one enabled `ha_govee_led_ble` entry.

The non-hardware workflow is covered by:

```bash
mkdir -p "$PWD/.harness/test-tmp"
