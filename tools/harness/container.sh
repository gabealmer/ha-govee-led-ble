...
elif [ "${HA_CONTAINER_PODMAN_PRIVILEGED:-0}" = 1 ]; then
  PODMAN_COMMAND=(sudo -n "${PODMAN:-/usr/bin/podman}")
else
...
INTEGRATION_DIR="${HA_GOVEE_LED_BLE_INTEGRATION_DIR:-$REPO_DIR/custom_components/ha_govee_led_ble}"
INTEGRATION_IS_PACKAGE=0
[ -z "${HA_GOVEE_LED_BLE_INTEGRATION_DIR:-}" ] || INTEGRATION_IS_PACKAGE=1
FRONTEND_DIR="$REPO_DIR/frontend"

ensure_runtime_build() {
  local bootstrap filename
  while IFS= read -r filename; do
    case "$filename" in ""|\#*) continue ;; esac
    [ -f "$INTEGRATION_DIR/generated_protocol/$filename" ] || {
      [ "$INTEGRATION_IS_PACKAGE" = 0 ] || {
        echo "local integration package is missing generated_protocol/$filename" >&2
        return 1
      }
  ...
  [ -f "$INTEGRATION_DIR/frontend/manifest.json" ] || {
    [ "$INTEGRATION_IS_PACKAGE" = 0 ] || {
      echo "local integration package is missing frontend/manifest.json" >&2
      return 1
    }
    make -C "$REPO_DIR" build
    return
  ...
  bootstrap="$(
    python3 - "$INTEGRATION_DIR/frontend/manifest.json" <<'PY'
import json
import sys
from pathlib import Path

value = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8")).get("bootstrap")
print(value if value == "effect-studio-bootstrap.js" else "")
PY
  )"
  if [ -z "$bootstrap" ] || [ ! -f "$INTEGRATION_DIR/frontend/$bootstrap" ]; then
    [ "$INTEGRATION_IS_PACKAGE" = 0 ] || {
      echo "local integration package has an invalid frontend manifest" >&2
      return 1
    }
    make -C "$REPO_DIR" build
  fi
...
if { [ "$COMMAND" = up ] || [ "$COMMAND" = restart ]; } && [ "$DRY_RUN" != 1 ]; then
  ensure_runtime_build
fi
