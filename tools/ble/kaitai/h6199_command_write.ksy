  ...
  scene_body:
    ...
    seq:
      ...
      - id: source
        type: u1
        doc: |
          [CONFIRMED_LIVE] where this scene came from, at frame offset 5. Three values are
          captured: 1 for Sunrise, Sunset and Candlelight; 2 for the twelve catalogue scenes
          across five galleries; and 0 for two effects generated by the app's AI Lighting Bot.

          IT WAS PREVIOUSLY NAMED FOR WHETHER THE LIGHT ALREADY HELD THE SCENE, and that is
          wrong. The argument was a clean correlation: every scene written with 2 was
          preceded by a multi-frame 0xA3 upload of a definition, and every scene written with
          1 by no A3 traffic at all, over fifteen scenes without exception. The AI effects
          break it. They are uploaded exactly like the catalogue ones, seven frames then five,
          and they carry 0 rather than 2, so the byte cannot be reporting whether an upload
          was needed. The correlation was real and it was a consequence: the scenes the light
          already holds are the ones that need no upload, not the ones this byte marks.

          What it does track, on eighteen samples, is PROVENANCE: built into the firmware,
          fetched from the catalogue, or generated. That reading is not isolated either, and
          it is deliberately not in the field name. The captures that would settle it are an
          AI effect the light has been given before, which separates provenance from novelty,
          and a catalogue scene applied twice in one session, which separates it from whether
          this particular light has seen the scene already.

          The three scenes carrying 1 have small numbers, 0, 1 and 9, which fits a table held
          in firmware. That is a pattern in three samples and not evidence: do not read a
          small number as meaning built-in.
      - id: opaque_tail
        size: 13
